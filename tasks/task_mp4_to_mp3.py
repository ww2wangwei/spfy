...
def convert_mp4_to_mp3(mp4_files: List[str], ffmpeg_path: str = None, 
    ...
    for i, mp4_path in enumerate(mp4_files, start=1):
        try:
            ...
            output_path = mp4_path.parent / f"{mp4_path.stem}.mp3"
            
            if log_callback:
                log_callback(f"[{i}/{total_files}] Converting: {mp4_path.name}")
            
            def log_line(line):
                if log_callback:
                    log_callback(f"  {line}")
            
            ffmpeg.extract_audio(str(mp4_path), str(output_path), callback=log_line)
            
            if log_callback:
                log_callback(f"  ✓ Successfully converted to: {output_path.name}")
